...
- Updated the `vita init` starter `AGENTS.md` bootstrap so agents install Vita
  skills into the appropriate project-local skill location, force-refresh stale
  copies before install, and report installed skill names, paths, and visible
  in-session availability back to the user.
