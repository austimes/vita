...
def test_compare_versions_orders_dotted_numbers():
    assert handlers._compare_versions("0.3.0", "0.4.1") == -1
    assert handlers._compare_versions("0.4.1", "0.4.1") == 0
    assert handlers._compare_versions("0.4.2", "0.4.1") == 1
...
def test_run_update_command_is_noop_when_current(monkeypatch, capsys):
    ...
    monkeypatch.setattr(handlers, "_fetch_latest_tool_version", lambda: "0.4.1")
    ...
    assert calls == []
    assert "already up to date (version 0.4.1)" in capsys.readouterr().out
...
def test_run_update_command_invokes_uv_when_remote_is_newer(monkeypatch, capsys):
    ...
    assert "Updating vita tool package from GitHub main (0.3.0 -> 0.4.1)." in stdout
...
def test_run_update_command_skips_when_installed_is_newer(monkeypatch, capsys):
    ...
    assert excinfo.value.code == 0
    assert "newer than GitHub main (0.4.2 > 0.4.1)" in capsys.readouterr().out
