...
def setup_work_dir(
    ...
    run_option_lines: list[str] | None = None,
) -> Path:
    ...
    _write_run_option_include(work_dir, run_option_lines)

    return work_dir


def _write_run_option_include(
    work_dir: Path,
    run_option_lines: list[str] | None,
) -> None:
    """Inject optional RUN-file statements into the copied scenario scaffold."""
    if not run_option_lines:
        return

    include_path = work_dir / "vita_run_options.inc"
    include_path.write_text(
        "\n".join(
            [
                "* Generated by veda_run_times for run-scoped reporting options",
                *run_option_lines,
                "",
            ]
        ),
    ...
    scenario_path = work_dir / "scenario.run"
    scenario_text = scenario_path.read_text(encoding="utf-8")
    marker = "$   BATINCLUDE initmty.mod"
    if marker not in scenario_text:
        raise ValueError(
            f"Unable to inject run options into scenario.run: missing marker {marker!r}"
        )
