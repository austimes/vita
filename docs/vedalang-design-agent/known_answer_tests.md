...
5. For KA03 emissions-flow coverage, extract flows with `include_flows=True`, assert commodity-token flow/activity ratios from solved outputs, and accept deterministic `VAR_FLO`/`PAR_FLO` source fallback via `results.var_flo_source`.
...
4. When flow symbol availability varies across fixture shapes, prefer commodity-token solved-behavior checks and allow deterministic `VAR_FLO`/`PAR_FLO` fallback while preserving `flow_source` context in assertion failures.
