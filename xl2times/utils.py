...
def require_table(
    ...
    *,
...
) -> datatypes.EmbeddedXlTable | None:
    """Safely get a required table, emitting diagnostic if missing.

    Unlike single_table(), this function returns None instead of raising when
    the table is missing, allowing callers to handle the absence gracefully.
    ...
    tables
        List of tables in EmbeddedXlTable format.
    tag
        Tag name (e.g., Tag.currencies or "~CURRENCIES").
    feature
        Optional feature name for better error messages (e.g., "currency validation").
    emit_diagnostic
        If True, emit a structured diagnostic when the table is missing.

    Returns
    -------
    datatypes.EmbeddedXlTable | None
        Table with the given tag, or None if not found.
    """
    tag_str = tag.value if hasattr(tag, "value") else str(tag)
    table = next((t for t in tables if t.tag == tag_str), None)

    if table is None and emit_diagnostic:
        feature_context = f" (required for {feature})" if feature else ""
        get_collector().error(
            "MISSING_REQUIRED_TABLE",
            f"Required table {tag_str} is missing{feature_context}",
            context={"expected_tag": tag_str, "feature": feature},
            suggestion=f"Add a {tag_str} table to your model",
        )
        logger.warning(f"Required table {tag_str} is missing{feature_context}")
...
def require_column(
    ...
    *,
    ...
    """Safely get a required column from a table, emitting diagnostic if missing.
    ...
    colname
        Column name to return the values of.
    feature
        Optional feature name for better error messages.
    emit_diagnostic
        If True, emit a structured diagnostic when table/column is missing.

    Returns
    -------
    numpy.ndarray | None
        Values for the column, or None if table/column not found.
    """
    table = require_table(tables, tag, feature=feature, emit_diagnostic=emit_diagnostic)
    if table is None:
        return None
    ...
    if colname not in df.columns:
        if emit_diagnostic:
            ...
            get_collector().error(
                ...
                f"Required column '{colname}' is missing from table {tag_str}{feature_context}",
                context={"table_tag": tag_str, "column": colname, "feature": feature},
                suggestion=f"Add a '{colname}' column to the {tag_str} table",
            )
            logger.warning(
                f"Required column '{colname}' is missing from table {tag_str}{feature_context}"
            )
...
def require_scalar(
    ...
    *,
    ...
    """Safely get a scalar value from a table, emitting diagnostic if missing.

    Parameters
    ----------
    table_tag
        Tag name for the table containing the scalar.
    tables
        List of tables in EmbeddedXlTable format.
    feature
        Optional feature name for better error messages.
    emit_diagnostic
        If True, emit a structured diagnostic when the table is missing or invalid.

    Returns
    -------
    Any | None
        The scalar value, or None if not found or invalid.
    """
    tag_str = table_tag.value if hasattr(table_tag, "value") else str(table_tag)
    table = require_table(
        tables, table_tag, feature=feature, emit_diagnostic=emit_diagnostic
    )
    if table is None:
        return None
    ...
    if df.shape[0] != 1 or df.shape[1] != 1:
        if emit_diagnostic:
            feature_context = f" (required for {feature})" if feature else ""
            get_collector().error(
                "INVALID_SCALAR_TABLE",
                f"Table {tag_str} should contain exactly one value{feature_context}",
                context={
                    "table_tag": tag_str,
                    "actual_shape": list(df.shape),
                    "feature": feature,
                },
                suggestion=f"Ensure {tag_str} table has exactly one cell with a value",
            )
            logger.warning(
                f"Table {tag_str} has invalid shape {df.shape} for scalar{feature_context}"
            )
        return None
...
def require_df_column(
    df: DataFrame,
    colname: str,
    *,
    context_name: str = "DataFrame",
    feature: str | None = None,
    emit_diagnostic: bool = True,
) -> numpy.ndarray | None:
    """Safely get a column from a DataFrame, emitting diagnostic if missing.
    ...
    colname
        Column name to return the values of.
    context_name
        Name to use in error messages (e.g., "model.processes").
    feature
        Optional feature name for better error messages.
    emit_diagnostic
        If True, emit a structured diagnostic when column is missing.

    Returns
    -------
    numpy.ndarray | None
        Values for the column, or None if column not found.
    """
    if colname not in df.columns:
        if emit_diagnostic:
            feature_context = f" (required for {feature})" if feature else ""
            get_collector().error(
                "MISSING_REQUIRED_COLUMN",
                f"Required column '{colname}' is missing from {context_name}{feature_context}",
                context={
                    "source": context_name,
                    "column": colname,
                ...
                suggestion=f"Ensure the '{colname}' column exists in your process/commodity definitions",
            )
            logger.warning(
                f"Required column '{colname}' is missing from {context_name}{feature_context}"
            )
