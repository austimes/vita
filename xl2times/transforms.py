...
from .diagnostics import Severity, get_collector
...
def remove_tables_with_formulas(
    ...
    def has_formulas(table):
        ...
        if has:
            ...
            get_collector().warning(
                "TABLE_HAS_FORMULAS",
                f"Table {table.tag} contains Excel formulas and was excluded",
                file=table.filename,
                sheet=table.sheetname,
                range=table.range,
                tag=table.tag,
                suggestion="Replace formulas with static values",
            )
        return has
...
def validate_input_tables(
    ...
    for table in tables:
        if not Tag.has_tag(table.tag):
            ...
            get_collector().warning(
                "UNKNOWN_TAG",
                f"Unrecognized tag '{table.tag}' - table was dropped",
                file=table.filename,
                sheet=table.sheetname,
                range=table.range,
                tag=table.tag,
                suggestion=f"Check if '{table.tag}' is a valid VEDA tag",
            )
            continue
        if discard(table):
            continue
        ...
        if len(dupes) > 0:
            ...
            get_collector().warning(
                "DUPLICATE_COLUMNS",
                f"Table has duplicate column names: {','.join(dupes)}",
                file=table.filename,
                sheet=table.sheetname,
                range=table.range,
                tag=table.tag,
                context={"duplicate_columns": list(dupes)},
                suggestion="Rename duplicate columns to be unique",
            )
        result.append(table)
    return result
        ...
        if required_cols:
            ...
            if missing_cols:
                ...
                get_collector().error(
                    ...
                    f"Table {tag.value} is missing required columns: {', '.join(missing_cols)}",
                    file=table.filename,
                    sheet=table.sheetname,
                    range=table.range,
                    tag=table.tag,
                    context={"missing_columns": list(missing_cols), "required_columns": list(required_cols)},
                    suggestion=f"Add the missing columns: {', '.join(missing_cols)}",
                )
                # Discard the table
                continue
            ...
            case Tag.fi_process:
                ...
                if "sets" not in df.columns:
                    get_collector().error(
                        ...
                        "Required column 'sets' is missing from ~FI_PROCESS table",
                        context={
                            "source": "~FI_PROCESS",
                            "column": "sets",
                            "available_columns": list(df.columns),
                        },
                        suggestion="Add a 'Sets' column to the ~FI_PROCESS table",
                    )
                    model.processes = df
                else:
...
def process_flexible_import_tables(
    ...
    currencies = utils.require_column(
        tables, Tag.currencies, "currency", feature="currency validation"
    )

    # Build timeslice set from ts_tslvl if available, else empty
    if not model.ts_tslvl.empty and "tslvl" in model.ts_tslvl.columns and "ts" in model.ts_tslvl.columns:
        timeslice_values = set(model.ts_tslvl["tslvl"]).union(model.ts_tslvl["ts"])
    else:
        timeslice_values = set()
        get_collector().add(
            severity=Severity.WARNING,
            code="MISSING_TIMESLICES",
            message="No timeslice definitions found; timeslice validation disabled",
        )

    legal_values = {
        "limtype": set(config.times_sets["LIM"]),
        "timeslice": timeslice_values,
        "commodity": set(utils.merge_columns(tables, Tag.fi_comm, "commodity")),
        "region": model.internal_regions,
        "currency": currencies if currencies is not None else [],
...
def process_user_constraint_tables(
    ...
    list[EmbeddedXlTable]
        List of tables in EmbeddedXlTable format with all FI_T processed.
    """
    # Build timeslice set safely
    if not model.ts_tslvl.empty and "tslvl" in model.ts_tslvl.columns:
        uc_timeslice_values = set(model.ts_tslvl["tslvl"])
    else:
        uc_timeslice_values = set()
...
def fill_in_column_defaults(
    ...
    brm_table = next((t for t in tables if t.tag == Tag.book_regions_map), None)
    if brm_table is None:
        get_collector().error(
            "MISSING_BOOKREGIONS_MAP",
            "~BOOKREGIONS_MAP table is missing; VT_* files will use all internal regions as fallback",
            context={"expected_tag": Tag.book_regions_map.value},
            suggestion="Add a ~BOOKREGIONS_MAP table to define region mappings for VT_* files",
        )
        logger.warning(
            "~BOOKREGIONS_MAP table is missing; VT_* files will use all internal regions"
        )
    else:
        brm = brm_table.dataframe
...
def create_model_units(
    ...
    # Activity units from processes
    tact_vals = utils.require_df_column(
        model.processes, "tact", context_name="model.processes", feature="units"
    )
    if tact_vals is not None:
        units_parts.append(pd.DataFrame({"unit": pd.unique(tact_vals), "type": "activity"}))

    # Capacity units from processes
    tcap_vals = utils.require_df_column(
        model.processes, "tcap", context_name="model.processes", feature="units"
    )
    if tcap_vals is not None:
        units_parts.append(pd.DataFrame({"unit": pd.unique(tcap_vals), "type": "capacity"}))

    # Commodity units
    unit_vals = utils.require_df_column(
        model.commodities, "unit", context_name="model.commodities", feature="units"
    )
    if unit_vals is not None:
        units_parts.append(pd.DataFrame({"unit": pd.unique(unit_vals), "type": "commodity"}))

    # Currency units
    if Tag.currencies in tables and "currency" in tables[Tag.currencies].columns:
        units_parts.append(
            pd.DataFrame(
                {"unit": tables[Tag.currencies]["currency"].unique(), "type": "currency"}
            )
        )

    if units_parts:
        model.units = pd.concat(units_parts)
    else:
        model.units = pd.DataFrame(columns=["unit", "type"])
...
def process_time_periods(
    ...
    start_year = utils.require_scalar(
        Tag.start_year, tables, feature="time period processing"
    )
    ...
    if active_pdef is None:
        return tables

    time_periods_table = utils.require_table(
        tables, Tag.time_periods, feature="time period processing"
    )
...
def process_regions(
    ...
    region_table = utils.require_table(
        tables, Tag.book_regions_map, feature="region processing"
    )
    if region_table is None:
        return tables

    region_def = region_table.dataframe
    region_def = (
...
def prepare_for_querying(
    ...
    if Tag.currencies in tables and "currency" in tables[Tag.currencies].columns:
        currency = tables[Tag.currencies]["currency"].iloc[0] if len(tables[Tag.currencies]) > 0 else "USD"
    else:
...
def include_cgs_in_topology(
    ...
    # Handle missing fi_process table
    if Tag.fi_process not in tables:
        return tables
...
def create_model_cgs(
    ...
    # Check required columns exist in commodities
    required_comm_cols = ["region", "commodity"]
    missing_comm_cols = [c for c in required_comm_cols if c not in model.commodities.columns]
    if missing_comm_cols:
        get_collector().error(
            "MISSING_REQUIRED_COLUMN",
            f"Required column(s) {missing_comm_cols} missing from model.commodities",
            context={
                "source": "model.commodities",
                "missing_columns": missing_comm_cols,
                "available_columns": list(model.commodities.columns),
                "feature": "commodity groups",
            },
            suggestion="Ensure ~FI_COMM table has 'CommName' (maps to 'commodity') and 'Region' columns",
        )
        model.commodity_groups = pd.DataFrame(columns=["region", "commodity", "commoditygroup", "gmap"])
        return tables
    ...
    # Check if topology has commoditygroup column
    if "commoditygroup" not in model.topology.columns:
        get_collector().warning(
            "MISSING_TOPOLOGY_COLUMN",
            "Column 'commoditygroup' is missing from model.topology - using only single-member CGs",
            context={
                "source": "model.topology",
                "available_columns": list(model.topology.columns),
            },
        )
        cgs = single_cgs
    else:
...
def complete_model_trade(
    ...
    # Check required columns exist
    if "sets" not in model.processes.columns:
        get_collector().error(
            "MISSING_REQUIRED_COLUMN",
            "Required column 'sets' is missing from model.processes",
            context={
                "source": "model.processes",
                "column": "sets",
                "available_columns": list(model.processes.columns),
                "feature": "trade processing",
            },
            suggestion="Ensure ~FI_PROCESS table has a 'Sets' column",
        )
        return tables
    ...
    top_ire = top_ire.reset_index(drop=True)
    ire_prc = ire_prc.reset_index(drop=True)
...
def fill_in_missing_pcgs(
    ...
    # Handle empty or missing primarycg column
    if df.empty or "primarycg" not in df.columns:
        return tables
...
def process_processes(
    ...
    original_dfs = []
    # Check for DAYNITE in timeslice levels, handling empty ts_tslvl
    if not model.ts_tslvl.empty and "tslvl" in model.ts_tslvl.columns:
        daynite = "DAYNITE" in set(model.ts_tslvl["tslvl"])
    else:
        daynite = False
    ...
    if original_dfs:
        ...
        model.custom_psets = pd.DataFrame(columns=["sets", "process"])
...
def create_model_topology(
    ...
    if not top_info:
        model.topology = pd.DataFrame(columns=["region", "process", "io", "commodity"])
        return tables
...
def generate_topology_dictionary(
    ...
    # Check for required columns in processes
    required_pros_cols = ["process", "sets"]
    missing_pros_cols = [c for c in required_pros_cols if c not in pros.columns]
    if missing_pros_cols:
        get_collector().error(
            "MISSING_REQUIRED_COLUMN",
            f"Required column(s) {missing_pros_cols} missing from model.processes",
            context={
                "source": "model.processes",
                "missing_columns": missing_pros_cols,
                "available_columns": list(pros.columns),
            },
            suggestion="Ensure ~FI_PROCESS table has 'TechName' (maps to 'process') and 'Sets' columns",
        )
        return dictionary

    # Check for required columns in commodities
    required_coms_cols = ["commodity", "csets"]
    missing_coms_cols = [c for c in required_coms_cols if c not in coms.columns]
    if missing_coms_cols:
        get_collector().error(
            "MISSING_REQUIRED_COLUMN",
            f"Required column(s) {missing_coms_cols} missing from model.commodities",
            context={
                "source": "model.commodities",
                "missing_columns": missing_coms_cols,
                "available_columns": list(coms.columns),
            },
            suggestion="Ensure ~FI_COMM table has 'CommName' (maps to 'commodity') and 'Csets' columns",
        )
        return dictionary
    ...
    # Check topology has required columns
    required_topology_cols = ["process", "commodity", "io"]
    missing_topology_cols = [
        c for c in required_topology_cols if c not in model.topology.columns
    ]
    if missing_topology_cols:
        get_collector().error(
            "MISSING_REQUIRED_COLUMN",
            f"Required column(s) {missing_topology_cols} missing from model.topology",
            context={
                "source": "model.topology",
                "missing_columns": missing_topology_cols,
                "available_columns": list(model.topology.columns),
            },
            suggestion="Ensure ~FI_T table has TechName, Comm-IN/Comm-OUT columns",
        )
        return dictionary
    ...
    # Add optional entries only if columns exist
    if "description" in pros.columns:
        dict_info.append(
            {
    ...
    if "description" in coms.columns:
        dict_info.append(
            {
...
def apply_transform_tables(
    ...
    if "module_name" in model.processes.columns and "process" in model.processes.columns:
    ...
    if "module_name" in model.commodities.columns and "commodity" in model.commodities.columns:
    ...
    obj_region = dict()
    if all(c in model.processes.columns for c in ["region", "process"]):
        obj_region["process"] = model.processes[["region", "process"]].drop_duplicates()
    else:
        obj_region["process"] = pd.DataFrame(columns=["region", "process"])

    if all(c in model.commodities.columns for c in ["region", "commodity"]):
        obj_region["commodity"] = model.commodities[
            ["region", "commodity"]
        ].drop_duplicates()
    else:
        obj_region["commodity"] = pd.DataFrame(columns=["region", "commodity"])
...
def assign_model_attributes(
    ...
    if Tag.fi_t not in tables:
        model.attributes = pd.DataFrame()
        return tables
    ...
    if Tag.uc_t in tables.keys():
        ...
        if ts_uc_attrs and not model.ts_tslvl.empty and "ts" in model.ts_tslvl.columns:
            ...
            if any(index) and "tslvl" in model.ts_tslvl.columns:
...
def complete_processes(
    ...
    # Skip if model.processes is empty or missing required columns
    if model.processes.empty or "region" not in model.processes.columns or "process" not in model.processes.columns:
        return tables

    # Skip if model.trade is empty
    if model.trade.empty:
        return tables
...
def apply_final_fixup(
    ...
    # Handle missing topology columns gracefully
    required_cols = ["region", "process", "commodity"]
    if all(c in model.topology.columns for c in required_cols):
        reg_com_flows = model.topology[required_cols].drop_duplicates(ignore_index=True)
    else:
        reg_com_flows = pd.DataFrame(columns=required_cols)
    ...
    # Fill other_indexes for COST (alias of IRE_PRICE)
    # Support both canonical name (IRE_PRICE) and alias (COST)
    cost_mapping = {"MIN": "IMP", "EXP": "EXP", "IMP": "IMP"}
    cost_index = df["original_attr"].isin({"COST", "IRE_PRICE"}) & df["process"].notna()
    ...
    has_original_attr = "original_attr" in df.columns
    has_commodity = "commodity" in df.columns
    has_other_indexes = "other_indexes" in df.columns

    if has_original_attr and has_commodity and has_other_indexes:
        index = df["original_attr"].isin({"COST", "IRE_PRICE"})
        if any(index):
            if "commodity-in" in df.columns:
                i_exp = index & (df["other_indexes"] == "EXP")
                df.loc[i_exp, "commodity"] = df.loc[i_exp, "commodity-in"]
            if "commodity-out" in df.columns:
                i_imp = index & (df["other_indexes"] == "IMP")
                df.loc[i_imp, "commodity"] = df.loc[i_imp, "commodity-out"]

    # Fill Commodity for COST/IRE_PRICE if missing
    if not has_original_attr or not has_commodity:
        pass  # Skip if columns don't exist
    elif any(df["original_attr"].isin({"COST", "IRE_PRICE"}) & df["commodity"].isna()):
        i_com_na = df["original_attr"].isin({"COST", "IRE_PRICE"}) & df["commodity"].isna()
        ...
        i_cost = df["original_attr"].isin({"COST", "IRE_PRICE"})
    ...
    if "sets" not in model.processes.columns or "process" not in model.processes.columns:
        ire_set = set()  # Can't determine IRE processes without sets column
    else:
    ...
    if (
        not config.ie_override_in_syssettings
        and "year" in df.columns
        and "module_type" in df.columns
    ):
        df = df.reset_index(drop=True)
