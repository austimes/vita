...
try:
    ...
    __version__ = pkg_version("xl2times")
except Exception:
    __version__ = "unknown"
...
def read_xl(
    ...
    # Register tables with manifest builder if provided
    if manifest_builder:
        for table in raw_tables:
            manifest_builder.add_input_file(table.filename)
            manifest_builder.add_table(table)
    ...
    # Early validation transforms (can stop here for diagnostics-only runs)
    validation_transforms = [
        ...
        transforms.remove_tables_with_formulas,
        transforms.normalize_column_aliases,
        transforms.remove_comment_rows,
        transforms.revalidate_input_tables,
    ]

    # Run validation transforms
    input = raw_tables
    for transform in validation_transforms:
        start_time_t = time.time()
        output = transform(config, input, model)
        end_time_t = time.time()
        logger.opt(raw=True).debug(_log_sep)
        logger.info(
            f"transform {transform.__code__.co_name} took {end_time_t - start_time_t:.2f} seconds"
        )
        logger.opt(raw=True).debug(_log_sep)
        input = output
    ...
    if stop_after_validate:
        if output_dir:
            dump_tables(input, os.path.join(output_dir, "validated_tables.txt"))
        return model, config
    ...
    # Continue from validation output (input variable already set)
    output = {}
...
def run(args: argparse.Namespace) -> str | None:
    ...
    # Initialize diagnostics collector if JSON output requested
    diagnostics_collector = get_collector()
    if args.diagnostics_json:
        diagnostics_collector.enable()

    # Initialize manifest builder if JSON output requested
    manifest_builder = None
    if args.manifest_json:
        manifest_builder = ManifestBuilder(case=args.case)
    ...
    if args.only_read:
        ...
        if args.manifest_json and manifest_builder:
            manifest_builder.write_json(args.manifest_json, model, __version__)
        if args.diagnostics_json:
            diagnostics_collector.write_json(args.diagnostics_json, __version__)
        sys.exit(0)
    ...
    # If we stopped after validation, just output diagnostics and exit
    if args.stop_after_validate:
        if args.manifest_json and manifest_builder:
            manifest_builder.write_json(args.manifest_json, model, __version__)
            logger.info(f"Manifest written to {args.manifest_json}")
        if args.diagnostics_json:
            diagnostics_collector.write_json(args.diagnostics_json, __version__)
            logger.info(f"Diagnostics written to {args.diagnostics_json}")
        sys.exit(0)
...
def parse_args(arg_list: None | list[str]) -> argparse.Namespace:
    ...
    args_parser.add_argument(
        "--stop_after_validate",
        action="store_true",
        help="Stop after initial validation transforms (useful for diagnostics testing)",
    )
    args_parser.add_argument(
        "--no_cache",
        action="store_true",
    ...
    args_parser.add_argument(
        ...
        metavar="PATH",
        help="Write machine-readable diagnostics to a JSON file",
    )
    args_parser.add_argument(
        "--manifest-json",
        type=str,
        dest="manifest_json",
        metavar="PATH",
        help="Write a manifest describing parsed tables and symbols to a JSON file",
    )
    args_parser.add_argument(
        "-v",
        "--verbose",
